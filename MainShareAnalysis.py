...
class StockAnalyzer:
    ...
    def _get_all_raw_data(self) -> Dict[str, pd.DataFrame]:
        ...
        data = {
            ...
            'market_fund_flow_raw': self.data_fetcher.fetch(ak.stock_fund_flow_individual, "5日市场资金流向",
                                                            symbol="5日排行"),
            'market_fund_flow_raw_10': self.data_fetcher.fetch(ak.stock_fund_flow_individual, "10日市场资金流向",
                                                               symbol="10日排行"),
            'market_fund_flow_raw_20': self.data_fetcher.fetch(ak.stock_fund_flow_individual, "20日市场资金流向",
                                                               symbol="20日排行"),
            'strong_stocks_raw': self.data_fetcher.fetch(ak.stock_zt_pool_strong_em, "强势股池",
                                                         date=datetime.now().strftime('%Y%m%d')),
            'consecutive_rise_raw': self.data_fetcher.fetch(ak.stock_rank_lxsz_ths, "连续上涨"),
        ...
        data['xstp_10_raw'] = self.data_fetcher.fetch(ak.stock_rank_xstp_ths, "向上突破10日均线", symbol="10日均线")
        data['xstp_30_raw'] = self.data_fetcher.fetch(ak.stock_rank_xstp_ths, "向上突破30日均线", symbol="30日均线")
        data['xstp_60_raw'] = self.data_fetcher.fetch(ak.stock_rank_xstp_ths, "向上突破60日均线", symbol="60日均线")
    ...
    def _consolidate_data(self, processed_data: Dict[str, pd.DataFrame],
        ...
        # ====== 新增：从所有可用的原始数据源(processed_data)中提取并关联股票简称 ======
        name_dfs = []
        ...
        if not spot_df.empty and '股票代码' in spot_df.columns:
            ...
            if '最新价' in spot_df.columns:
                final_df = pd.merge(final_df, spot_df[['股票代码', '最新价']].drop_duplicates(subset=['股票代码']),
                                    on='股票代码', how='left')
            else:
                final_df['最新价'] = 'N/A'
        else:
        ...
        if not industry_df.empty:
            ...
            if '股票简称' in industry_df.columns:
                ...
                final_df['股票简称'] = final_df.apply(
                    lambda row: ind_name_map.get(row['股票代码'], 'N/A') if pd.isna(row['股票简称']) or row['股票简称'] == 'N/A' else row['股票简称'],
                    axis=1
                )
        ...
        # --- 新增：计算多头排列评分 (精简版) --- #
        # 1. 获取历史K线数据 (确保 hist_df_all 已从 processed_data 或数据库中正确获取)
        ...
        if hist_df_all.empty:
            ...
            # 修改：不再设置 FullBull_Score 和 FullBull_Suggestion
            final_df['多头排列趋势'] = '趋势观望'
        else:
            # 3. 【修复】自适应检测日期列名（兼容 trade_date / date / 日期 等）
            ...
            if date_col_in_kline is None:
                self.logger.warning(
                    ...
                    f"实际列: {list(hist_df_all.columns)}，跳过评分。")
                # 修改：不再设置 FullBull_Score 和 FullBull_Suggestion
                final_df['多头排列趋势'] = '趋势观望'
            ...
            if date_col_in_kline is not None:
                ...
                def _compute_ma_if_missing(kline_df: pd.DataFrame) -> pd.DataFrame:
                    ...
                    for period in [5, 10, 20, 30, 60, 90, 120]:
                        ...
                        if col not in kline_df.columns:
                            kline_df[col] = kline_df['close'].rolling(window=period, min_periods=1).mean()
                    if 'MA_Volume_5' not in kline_df.columns:
                        kline_df['MA_Volume_5'] = kline_df['volume'].rolling(window=5, min_periods=1).mean()
                    return kline_df
                ...
                def calculate_bull_score_for_row(row):
                    ...
                    if code_col_in_kline == 'symbol':
                        stock_kline = hist_df_all[hist_df_all[code_col_in_kline].str[-6:] == stock_code]
                    else:
                        stock_kline = hist_df_all[hist_df_all[code_col_in_kline].str[:6] == stock_code]

                    if stock_kline.empty:
                        # 修改：不再返回 FullBull_Score 和 FullBull_Suggestion
                    ...
                    try:
                        ...
                        # 修改：只获取 level 和 status，忽略 score 和 suggestion
        ...
        for period, df_key, col_name in [
            ...
            # 找到资金流入相关的列名
            flow_col = next((col for col in ['净流入', '资金流入净额', '今日主力净流入-净额'] if col in fund_flow_df.columns), None)
            ...
            if not fund_flow_df.empty and '股票代码' in fund_flow_df.columns and flow_col:
                ...
                final_df = pd.merge(final_df, fund_flow_df[['股票代码', flow_col]].drop_duplicates(subset=['股票代码']),
                                    on='股票代码', how='left')
        ...
        # 资金流数据标准化处理
        f5_col, f10_col, f20_col = '5日资金流入万元', '10日资金流入万元', '20日资金流入万元'
        ...
        for col in ['MACD_12269', 'MACD_6135', 'MACD_组合背离', 'KDJ_Signal', 'CCI_Signal', 'RSI_Signal',
                    'BOLL_Signal']:
            if col in final_df.columns:
                final_df[col] = final_df[col].fillna('')
            else:
        ...
        # 重新排列列顺序
        base_cols = ['股票代码', '股票简称', '行业', '所属行业信号', '最新价', '主力成本', '主力成本差价', '成本位置',
                     '主力控盘强度']
        signal_cols = [
        ...
        report_cols = [
            '多头排列趋势',
            '资金动能', '5日资金流入万元', '10日资金流入万元', '20日资金流入万元'
        ]
        ...
        try:
            ...
            for sheet_name, df in sheets_data.items():
                ...
                for i, col in enumerate(df.columns):
                    ...
                    elif col in ['5日资金流入万元', '10日资金流入万元', '20日资金流入万元']:
                        # 确保资金流入列使用货币格式
                        worksheet.set_column(i, i, col_width, currency_format)
                    else:
    ...
    def _load_industry_info_from_generated_file(self, stock_codes_pure: List[str]) -> pd.DataFrame:
        """
        从生成的行业文件中加载行业信息
        """
        print("\n>>> 正在加载行业信息...")
        ...
        try:
            ...
            if not consolidated_report.empty:
                ...
                full_bull_score = pd.to_numeric(
                    ...
                    # 这里可能需要处理，因为 FullBull_Score 已被移除
                    errors='coerce'
                ).fillna(0)  # 由于 FullBull_Score 列不存在，fillna(0) 会创建一个全0的系列

                full_bull_level = consolidated_report.get('多头排列趋势', pd.Series(dtype=str))
                exempt_from_drop = (full_bull_level == '完全主升') | (full_bull_level == '趋势加速')
                ...
                drop_condition = (
                        ...
                        (dif_6135 < 0) &
                        kdj_is_empty &
                        (consolidated_report.get('5日资金流入万元', pd.Series(dtype=str)).astype(str).str.contains('-',
                                                                                                                   na=False)) &
